...
public class ForPrimitives64 {
  ...
  public static void encode(long[] longs, int bitsPerValue, DataOutput out, long[] tmp) throws IOException {
    ...
    if (bitsPerValue <= 8) {
      ...
      collapse8(numLongs, longs);
    } else if (bitsPerValue <= 16) {
      nextPrimitive = 16;
      numLongs = BLOCK_SIZE / 4;
      collapse16(numLongs, longs);
    } else {
      nextPrimitive = 32;
      numLongs = BLOCK_SIZE / 2;
      collapse32(numLongs, longs);
    }

    final int numLongsPerShift = bitsPerValue * FACTOR;
    int idx = 0;
  ...
  private static void collapse8(int numLongs, long[] arr) {
    for (int i = 0; i < numLongs; ++i) {
      arr[i] = (arr[i] << 56) | (arr[numLongs+i] << 48) | (arr[2* numLongs+i] << 40) | (arr[3*numLongs+i] << 32) |
          (arr[4*numLongs+i] << 24) | (arr[5*numLongs+i] << 16) | (arr[6*numLongs+i] << 8) | arr[7*numLongs+i];
    }
  }

  private static void collapse16(int numLongs, long[] arr) {
    for (int i = 0; i < numLongs; ++i) {
  ...
  public static void decodeSlow(int bitsPerValue, DataInput in, long[] tmp, long[] longs) throws IOException {
    final int numLongs = bitsPerValue * FACTOR;//bitsPerValue << 1;
    in.readLELongs(tmp, 0, numLongs);
