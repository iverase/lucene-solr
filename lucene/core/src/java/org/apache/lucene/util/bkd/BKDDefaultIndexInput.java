...
public class BKDDefaultIndexInput implements BKDIndexInput {
  ...
  public BKDIndexInput.IndexTree getIndexTree() {
    return new IndexTree(packedIndex.clone(), this.in.clone(), config, numLeaves, version, minPackedValue, maxPackedValue);
  }
  ...
  public LeafIterator getLeafTreeIterator() throws IOException {
    ...
    final IndexTree indexTree =
        new IndexTree(packedIndex.clone(), input, config, numLeaves, version, 1, 1, minPackedValue, maxPackedValue);
  ...
  private static class IndexTree implements BKDIndexInput.IndexTree {
    ...
    private IndexTree(
        ...
        int version,
        int nodeID,
        int level,  byte[] minPackedValue, byte[] maxPackedValue) {
      this.config = config;
    ...
    public BKDIndexInput.IndexTree clone() {
      BKDDefaultIndexInput.IndexTree index =
              new BKDDefaultIndexInput.IndexTree(
                      ...
                      version,
                      nodeID,
                      level, minPackedValueStack[level], maxPackedValueStack[level]);
      // copy node data
    ...
    private void pushLeft() {
      ...
      System.arraycopy(maxPackedValueStack[level-1], 0, maxPackedValueStack[level], 0, config.packedIndexBytesLength);
      System.arraycopy(minPackedValueStack[level-1], 0, minPackedValueStack[level], 0, config.packedIndexBytesLength);
...
//          assert Arrays.compare(splitValuesStack[level - 1], splitDims[level] * config.bytesPerDim, splitDims[level] * config.bytesPerDim + config.bytesPerDim,
//                  minPackedValueStack[level-1], splitDims[level] * config.bytesPerDim, splitDims[level] * config.bytesPerDim + config.bytesPerDim) == 0 : isLeft;
          if (suffix > 0) {
            int firstDiffByteDelta = code / (1 + config.bytesPerDim);
            if (negativeDeltas[level * config.numIndexDims + splitDims[level]]) {
              firstDiffByteDelta = -firstDiffByteDelta;
            }
            int oldByte = splitValuesStack[level][splitDims[level] * config.bytesPerDim + prefix] & 0xFF;
            splitValuesStack[level][splitDims[level] * config.bytesPerDim + prefix] =
                (byte) (oldByte + firstDiffByteDelta);
            innerNodes.readBytes(
                splitValuesStack[level], splitDims[level] * config.bytesPerDim + prefix + 1, suffix - 1);
          } else {
