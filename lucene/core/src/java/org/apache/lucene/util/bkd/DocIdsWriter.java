...
class DocIdsWriter {
  ...
  private static void readSIMD(IndexInput in, int count, int[] docIDs, long[] tmp) throws IOException {
    final int iterations = count / ForUtilCheck.BLOCK_SIZE;
    final int bitsPerValue = in.readByte();
    for (int i = 0; i < iterations; ++i) {
      final long[] restored = new long[ForUtilCheck.BLOCK_SIZE];
      ForUtilCheck.decode(bitsPerValue, in, restored, tmp);
      for (int j = 0; j < ForUtilCheck.BLOCK_SIZE; ++j) {
        docIDs[i * ForUtilCheck.BLOCK_SIZE + j] = Math.toIntExact(restored[j]);
      }
    }
  }
  ...
  private static void readSIMD(IndexInput in, int count, IntersectVisitor visitor) throws IOException {
    final int iterations = count / ForUtilCheck.BLOCK_SIZE;
    final long[] tmp = new long[ForUtilCheck.BLOCK_SIZE / 2];
    final int bitsPerValue = in.readByte();
    for (int i = 0; i < iterations; ++i) {
      final long[] restored = new long[ForUtilCheck.BLOCK_SIZE];
      ForUtilCheck.decode(bitsPerValue, in, restored, tmp);
      for (int j = 0; j < ForUtilCheck.BLOCK_SIZE; ++j) {
        visitor.visit(Math.toIntExact(restored[j]));
      }
    }
  }
