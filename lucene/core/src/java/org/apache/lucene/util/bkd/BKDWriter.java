...
public class BKDWriter implements Closeable {
  ...
  private interface BKDTreeNodes {
  ...
  private long writeFieldNDims(IndexOutput out, String fieldName, MutablePointValues values) throws IOException {
    ...
    writeIndex(out, maxPointsInLeafNode, nodes);
    return indexFP;
  }
  ...
  */
  ...
  private byte[] packIndex(BKDTreeNodes nodes) throws IOException {
  ...
  private int recursePackIndex(ByteBuffersDataOutput writeBuffer, BKDTreeNodes nodes, long minBlockFP, List<byte[]> blocks,
    ...
    if (numLeaves == 1) {
      if (isLeft) {
        ...
        assert nodes.numLeaves() == numLeaves|| delta > 0 : "leaveOffset =" + leavesOffset;
      ...
      if (isLeft) {
        ...
        assert nodes.getLeafLP(leavesOffset) == minBlockFP;
  ...
  private void writeIndex(IndexOutput out, int countPerLeaf, BKDTreeNodes nodes) throws IOException {
  ...
  private void build(int leavesOffset, int numLeaves,
    ...
    if (numLeaves == 1) {
      ...
      final int newLeavesOffset = leavesOffset + numLeftLeafNodes;
      final int splitOffset = newLeavesOffset - 1;
      // set the split value
      ...
      build(newLeavesOffset, numLeaves - numLeftLeafNodes, reader, mid, to, out,
  ...
  private void build(int leavesOffset, int numLeaves,
    ...
    if (numLeaves == 1) {
      ...
      final int newLeaveOffset = leavesOffset + numLeftLeafNodes;
      final int splitValueOffset = newLeaveOffset - 1;
      ...
      build(newLeaveOffset, numLeaves - numLeftLeafNodes, slices[1],
