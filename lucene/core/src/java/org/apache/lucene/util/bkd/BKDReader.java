  ...
  private void addAll(IntersectState state, boolean grown) throws IOException {
    ...
    if (state.index.isLeafNode()) {
      ...
      if (state.index.nodeExists()) {
        final int numDocs = readDocIDs(state.in, state.index.getLeafBlockFP(), state.scratchIterator, state.scratchLongs);
        for (int i = 0; i < numDocs; i++) {
          state.visitor.visit(state.scratchIterator.docIDs[i]);
        }
      }
