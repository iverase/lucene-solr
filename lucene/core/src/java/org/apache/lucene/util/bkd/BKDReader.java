...
public final class BKDReader extends PointValues {
  ...
  private void addAll(IntersectState state, boolean grown) throws IOException {
    ...
    if (state.index.isLeafNode()) {
      ...
      if (state.index.nodeExists()) {
        visitDocIDs(state.in, state.index.getLeafBlockFP(), state.scratchIterator, state.visitor);
      }
  ...
  private void visitDocIDs(IndexInput in, long blockFP, BKDReaderDocIDSetIterator iterator, IntersectVisitor visitor) throws IOException {
    // Leaf node
    in.seek(blockFP);
