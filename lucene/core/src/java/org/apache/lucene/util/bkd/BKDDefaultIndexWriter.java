...
public class BKDDefaultIndexWriter implements BKDIndexWriter {
  ...
  @Override
  public void writeIndex(BKDConfig config, BKDInnerNodes nodes, int countPerLeaf,
                         byte[] minPackedValue, byte[] maxPackedValue, long pointCount, int numberDocs) throws IOException {
    final byte[] packedIndex = packIndex(config, nodes);
    final int numLeaves = nodes.numberOfLeaves();
  ...
  private byte[] packIndex(BKDConfig config,  BKDInnerNodes nodes) throws IOException {
    ...
    //System.out.println("\npack index");
    assert scratchOut.size() == 0;
    int totalSize = recursePackIndex(config, scratchOut, nodes, 0l, blocks, 1, lastSplitValues, new boolean[config.numIndexDims], false);
    scratchOut.reset();
   ...
   */
  private int recursePackIndex(BKDConfig config, ByteBuffersDataOutput writeBuffer,  BKDInnerNodes nodes, long minBlockFP, List<byte[]> blocks,
                               int nodeID, byte[] lastSplitValues, boolean[] negativeDeltas, boolean isLeft) throws IOException {
    if (nodeID >= nodes.numberOfLeaves()) {
      int leafID = nodeID - nodes.numberOfLeaves();
      //System.out.println("recursePack leaf nodeID=" + nodeID);

      // In the unbalanced case it's possible the left most node only has one child:
      if (leafID < nodes.numberOfLeaves()) {
        long delta = nodes.leafBlockFP(leafID) - minBlockFP;
        if (isLeft) {
          assert delta == 0;
          return 0;
        } else {
      ...
      long leftBlockFP;
      if (isLeft == false) {
        leftBlockFP = getLeftMostLeafBlockFP(nodes, nodeID);
      ...
      int splitDim = nodes.splitDimension(nodeID);//splitPackedValues[address++] & 0xff;
      ...
      BytesRef splitValue = nodes.splitPackedValue(nodeID);
      int prefix = Arrays.mismatch(splitValue.bytes, splitValue.offset, splitValue.offset + splitValue.length, lastSplitValues,
      ...
      if (prefix < config.bytesPerDim) {
        ...
        firstDiffByteDelta = (splitValue.bytes[splitValue.offset + prefix] & 0xFF) - (lastSplitValues[splitDim * config.bytesPerDim + prefix] & 0xFF);
      ...
      if (suffix > 1) {
        writeBuffer.writeBytes(splitValue.bytes, splitValue.offset + prefix + 1, suffix-1);
      }
      ...
      System.arraycopy(splitValue.bytes, splitValue.offset + prefix, lastSplitValues, splitDim * config.bytesPerDim + prefix, suffix);
      ...
      int leftNumBytes = recursePackIndex(config, writeBuffer, nodes, leftBlockFP, blocks, 2*nodeID, lastSplitValues, negativeDeltas, true);

      if (nodeID * 2 < nodes.numberOfLeaves()) {
      ...
      int rightNumBytes = recursePackIndex(config, writeBuffer, nodes, leftBlockFP, blocks, 2*nodeID+1, lastSplitValues, negativeDeltas, false);
  ...
  private long getLeftMostLeafBlockFP(BKDInnerNodes nodes, int nodeID) {
    ...
    while (nodeID < nodes.numberOfLeaves()) {
      nodeID *= 2;
    }
    int leafID = nodeID - nodes.numberOfLeaves();
    long result = nodes.leafBlockFP(leafID);
    if (result < 0) {
