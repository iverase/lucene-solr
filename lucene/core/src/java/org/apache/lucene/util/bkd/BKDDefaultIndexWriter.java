...
public class BKDDefaultIndexWriter implements BKDIndexWriter {
  ...
  public void writeLeafBlock(BKDConfig config, BKDLeafBlock leafBlock,
                             int[] commonPrefixes, int sortedDim, int leafCardinality, byte[] scratch) throws IOException {
  ...
  public void writeIndex(BKDConfig config, int countPerLeaf, long[] leafBlockFPs, byte[] splitPackedValues,
    ...
    out.writeVInt(config.numDataDims);
   ...
   */
  private int recursePackIndex(ByteBuffersDataOutput writeBuffer, BKDConfig config, long[] leafBlockFPs, byte[] splitPackedValues, long minBlockFP, List<byte[]> blocks,
    ...
    if (nodeID >= leafBlockFPs.length) {
      ...
      int leftNumBytes = recursePackIndex(writeBuffer, config, leafBlockFPs, splitPackedValues, leftBlockFP, blocks, 2*nodeID, lastSplitValues, negativeDeltas, true);
      ...
      int rightNumBytes = recursePackIndex(writeBuffer, config, leafBlockFPs, splitPackedValues, leftBlockFP, blocks, 2*nodeID+1, lastSplitValues, negativeDeltas, false);
  ...
  private void writeCommonPrefixes(DataOutput out, BKDConfig config, int[] commonPrefixes, BytesRef packedValue) throws IOException {
  ...
  private void writeLeafBlockPackedValues(DataOutput out, BKDConfig config, int[] commonPrefixLengths, int sortedDim, BKDLeafBlock packedValues, int leafCardinality, byte[] scratch) throws IOException {
    ...
    if (prefixLenSum == config.packedBytesLength) {
      ...
      // choose the cheaper serialization option.
      if (lowCardinalityCost <= highCardinalityCost) {
        out.writeByte((byte) -2);
        writeLowCardinalityLeafBlockPackedValues(out, config, commonPrefixLengths, packedValues, scratch);
      } else {
        out.writeByte((byte) sortedDim);
        writeHighCardinalityLeafBlockPackedValues(out, config, commonPrefixLengths, sortedDim, packedValues, compressedByteOffset);
      }
    }
  }
  ...
  private void writeLowCardinalityLeafBlockPackedValues(DataOutput out, BKDConfig config, int[] commonPrefixLengths, BKDLeafBlock packedValues, byte[] scratch) throws IOException {
    final int count = packedValues.count();
    if (config.numIndexDims != 1) {
      writeActualBounds(out, config, commonPrefixLengths, packedValues);
    }
    // do run-length compression on the full packed value
    ...
    for (int i = 1; i < count; i++) {
      ...
      for(int dim = 0; dim < config.numDataDims; dim++) {
        ...
        if (Arrays.mismatch(value.bytes, value.offset + start, value.offset + end, scratch, start, end) != -1) {
          ...
          for (int j = 0; j < config.numDataDims; j++) {
            out.writeBytes(scratch, j * config.bytesPerDim + commonPrefixLengths[j], config.bytesPerDim - commonPrefixLengths[j]);
          }
          System.arraycopy(value.bytes, value.offset, scratch, 0, config.packedBytesLength);
          cardinality = 1;
          break;
        } else if (dim == config.numDataDims - 1){
          cardinality++;
        }
      }
    }
    out.writeVInt(cardinality);
    for (int i = 0; i < config.numDataDims; i++) {
      out.writeBytes(scratch, i * config.bytesPerDim + commonPrefixLengths[i], config.bytesPerDim - commonPrefixLengths[i]);
    }
  }

  private void writeActualBounds(DataOutput out, BKDConfig config, int[] commonPrefixLengths, BKDLeafBlock packedValues) throws IOException {
  ...
  private void writeLeafBlockPackedValuesRange(DataOutput out, BKDConfig config, int[] commonPrefixLengths, int start, int end, BKDLeafBlock packedValues) throws IOException {
  ...
  private void writeHighCardinalityLeafBlockPackedValues(DataOutput out, BKDConfig config, int[] commonPrefixLengths, int sortedDim, BKDLeafBlock packedValues, int compressedByteOffset) throws IOException {
    if (config.numIndexDims != 1) {
      writeActualBounds(out, config, commonPrefixLengths, packedValues);
    }
    final int count = packedValues.count();
    ...
    for (int i = 0; i < count; ) {
      ...
      writeLeafBlockPackedValuesRange(out, config, commonPrefixLengths, i, i + runLen, packedValues);
      i += runLen;
      assert i <= count;
    }
  }
