...
public class SimpleTextBKDIndexWriter implements BKDIndexWriter {
  ...
  @Override
  public void writeLeafBlock(BKDConfig config, BKDLeafBlock leafBlock, int[] commonPrefixes, int sortedDim, int leafCardinality, byte[] scratch) throws IOException {
  ...
  public void writeIndex(BKDConfig config, int countPerLeaf, long[] leafBlockFPs, byte[] splitPackedValues, byte[] minPackedValue, byte[] maxPackedValue, long pointCount, int numberDocs) throws IOException {
    ...
    writeInt(out, config.numDataDims);
    newline(out);
